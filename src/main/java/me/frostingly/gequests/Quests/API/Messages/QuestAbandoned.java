...
public class QuestAbandoned {

    public void sendQuestAbandoned(Player p) {
        ...
        if (playerData != null) {
            if (!playerData.isExecuted()) {
                ...
                if (GEQuests.getInstance().getPlayerData().get(p.getUniqueId()).getQuest() != null) {
                    p.sendMessage(Utilities.format(GEQuests.getInstance().getConfig().getString("messages.quest_abandoned").replace("<quest>", ChatColor.stripColor(GEQuests.getInstance().getPlayerData().get(p.getUniqueId()).getQuest().getQuestName()))));
                }
                new BukkitRunnable() {
                    @Override
                    public void run() {
